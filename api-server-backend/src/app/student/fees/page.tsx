...
export default function StudentFeesPage() {
    ...
    return (
        <div className="w-full bg-[#f8fafc]/50 min-h-full pb-32">
            {/* Sticky Header Section */}
            <div className="sticky top-0 z-20 bg-[#f8fafc]/80 backdrop-blur-md px-5 py-4 border-b border-slate-100 flex flex-col gap-1">
                <h1 className="text-2xl font-black text-slate-800 flex items-center gap-3 tracking-tight">
                    <div className="w-10 h-10 bg-violet-50 rounded-2xl flex items-center justify-center shadow-sm border border-violet-100/50 shrink-0">
                        <Wallet className="w-5 h-5 text-violet-600" />
                    </div>
                    My Fee Records
                </h1>
                <p className="text-[11px] font-medium text-slate-500 tracking-wide leading-relaxed pl-1">
                ...
                {loading ? (
                    ...
                    <>
                        ...
                        <div className="sm:hidden divide-y divide-slate-100">
                            {fees.map((f: any) => {
                                const cfg = getStatusCfg(f.status);
                                const dueDate = f.due_date ? new Date(f.due_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
                                const displayAmount = Number(f.paid_amount) || Number(f.amount) || 0;
                                return (
                                    <div key={f.id} className="p-4">
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="flex items-center gap-1 text-slate-700">
                                                <span className="text-sm font-bold">₹{displayAmount.toLocaleString('en-IN')}</span>
                            ...
                            <table className="w-full text-sm">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {fees.map((f: any) => {
                                        const cfg = getStatusCfg(f.status);
                                        const dueDate = f.due_date ? new Date(f.due_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }) : '—';
                                        const displayAmt = Number(f.paid_amount) || Number(f.amount) || 0;
                                        return (
                                            <tr key={f.id} className="hover:bg-slate-50/50 transition-colors duration-200">
                                                ...
                                                <td className="py-4 px-6 text-right font-medium text-slate-600">₹{(Number(f.amount) || 0).toLocaleString('en-IN')}</td>
                                                <td className="py-4 px-6 text-right font-black text-slate-800">₹{displayAmt.toLocaleString('en-IN')}</td>
