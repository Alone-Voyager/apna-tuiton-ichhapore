            ...
            {loading ? (
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-100 rounded-xl p-4 md:p-6 shadow-sm relative overflow-hidden">
                    <div className="relative z-10">
                      <h2 className="text-2xl md:text-3xl font-extrabold text-blue-700 mb-1">{students.length}</h2>
                      <p className="text-xs md:text-sm font-semibold text-blue-600/80 uppercase tracking-wider">Total</p>
                    </div>
                  </div>

                  <div className="bg-gradient-to-br from-emerald-50 to-green-50 border border-emerald-100 rounded-xl p-4 md:p-6 shadow-sm relative overflow-hidden">
                    <div className="relative z-10">
                      <h2 className="text-2xl md:text-3xl font-extrabold text-emerald-700 mb-1">{presentCount}</h2>
                      <p className="text-xs md:text-sm font-semibold text-emerald-600/80 uppercase tracking-wider">Present</p>
                    </div>
                  </div>

                  <div className="bg-gradient-to-br from-rose-50 to-red-50 border border-red-100 rounded-xl p-4 md:p-6 shadow-sm relative overflow-hidden">
                    <div className="relative z-10">
                      <h2 className="text-2xl md:text-3xl font-extrabold text-rose-700 mb-1">{absentCount}</h2>
                      <p className="text-xs md:text-sm font-semibold text-rose-600/80 uppercase tracking-wider">Absent</p>
                    </div>
                  </div>

                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-orange-100 rounded-xl p-4 md:p-6 shadow-sm relative overflow-hidden">
                    <div className="relative z-10">
                      <h2 className="text-2xl md:text-3xl font-extrabold text-amber-700 mb-1">{leaveCount}</h2>
                      <p className="text-xs md:text-sm font-semibold text-amber-600/80 uppercase tracking-wider">Leave</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="bg-white rounded-lg p-4">
                  {students.length === 0 ? (
                    ...
                    <div className="grid grid-cols-1 gap-3 sm:gap-4 pb-20 sm:pb-0">
                      {filteredStudents.map((student) => {
                        const currentStatus = getAttendanceStatus(student.id);
                        return (
                          <div key={student.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between bg-white border border-slate-100 rounded-xl p-4 shadow-sm hover:shadow-md transition-all gap-3">
                            <div className="flex items-center gap-3 w-full sm:w-auto overflow-hidden">
                              <div className="w-10 h-10 shrink-0 rounded-full bg-indigo-50 flex items-center justify-center text-indigo-600 font-bold text-sm border border-indigo-100">
                                {student.name.split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
                              </div>
                              <div className="flex-1 min-w-0">
                                <h3 className="font-bold text-slate-800 text-[15px] truncate">{student.name}</h3>
                                <p className="text-xs font-medium text-slate-500 truncate mt-0.5">
                                  {student.roll_number}
                                  {student.classes && (
                                    <span className="ml-1.5 text-slate-400">• <span className="text-slate-500">{student.classes.name}</span></span>
                                  )}
                                </p>
                              </div>
                            </div>
                            <div className="flex gap-2 w-full sm:w-auto mt-1 sm:mt-0 pt-3 sm:pt-0 border-t border-slate-50 sm:border-0 justify-between sm:justify-end">
                              <button
                                onClick={() => handleAttendanceChange(student.id, 'Present')}
                                className={`flex-1 sm:flex-none px-4 py-2 sm:py-1.5 rounded-lg sm:rounded-full text-xs font-bold transition-all active:scale-95 ${currentStatus === 'Present'
                                    ? 'bg-emerald-500 text-white shadow-md shadow-emerald-200/50'
                                    : 'bg-white border text-emerald-600 border-emerald-200 hover:bg-emerald-50'
                                  }`}
                              >
                                Pre
                              </button>
                              <button
                                ...
                                className={`flex-1 sm:flex-none px-4 py-2 sm:py-1.5 rounded-lg sm:rounded-full text-xs font-bold transition-all active:scale-95 ${currentStatus === 'Absent'
                                    ? 'bg-rose-500 text-white shadow-md shadow-rose-200/50'
                                    : 'bg-white border text-rose-600 border-rose-200 hover:bg-rose-50'
                                  }`}
                              >
                                Abs
                              </button>
                              <button
                                ...
                                className={`flex-1 sm:flex-none px-4 py-2 sm:py-1.5 rounded-lg sm:rounded-full text-xs font-bold transition-all active:scale-95 ${currentStatus === 'Leave'
                                    ? 'bg-amber-500 text-white shadow-md shadow-amber-200/50'
                                    : 'bg-white border text-amber-600 border-amber-200 hover:bg-amber-50'
                                  }`}
                              >
                                Lev
                              </button>
                ...
                {/* Save Attendance */}
                <div className="fixed sm:relative bottom-[env(safe-area-inset-bottom)] sm:bottom-auto left-0 sm:left-auto right-0 sm:right-auto bg-white/90 sm:bg-white backdrop-blur-md sm:backdrop-blur-none border-t sm:border shadow-[0_-4px_10px_rgba(0,0,0,0.05)] sm:shadow-sm border-slate-200 sm:rounded-2xl p-4 lg:p-6 z-30 sm:z-10 mb-16 sm:mb-0">
                  <div className="flex flex-row items-center justify-between gap-4 max-w-lg mx-auto sm:max-w-none">
