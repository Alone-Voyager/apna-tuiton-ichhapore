  ...
  return (
    ...
    >
      ...
      <a
        ...
        onClick={handleClick}
        className={`block bg-white border-2 rounded-lg p-3 sm:p-4 transition-all ${
          isDragMode 
      ...
      >
        <h3 className={`font-semibold text-slate-800 mb-2 text-sm sm:text-base ${
          isDragMode ? '' : 'group-hover:text-red-600'
        }`}>
          {classInfo.name}
        </h3>
                  ...
                  <div className="flex flex-col space-y-3 sm:flex-row sm:items-center sm:space-y-0 sm:space-x-4">
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full sm:w-64 pl-10 pr-4 py-2.5 text-sm border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-500 focus:border-transparent"
                      />
                    </div>
                      ...
                      <div key={student.id} className="p-4 hover:bg-slate-50">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex-1">
                            <div className="font-medium text-slate-800 mb-1">{student.name}</div>
                            ...
                            {student.payment_months && student.payment_months.length > 0 && (
                              <div className="mt-2">
                                ...
                                <span className="text-xs text-slate-600">{student.payment_months.join(', ')}</span>
                              </div>
                            )}
                            {student.overdue_months && student.overdue_months.length > 0 && (
                              <div className="mt-2">
                                <span className="text-xs text-red-700 font-medium">Overdue: </span>
                                <span className="text-xs text-slate-600">{student.overdue_months.join(', ')}</span>
                              </div>
                            )}
                          </div>
                      ...
                      >
                        <div
                          className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-3 sm:gap-4 ${
                            isDragMode ? 'bg-blue-50 p-2 rounded-lg border-2 border-dashed border-blue-200' : ''
                          }`}
                        >
                          {isDragMode && (
          ...
          <div className="bg-white w-full max-w-md mx-4 rounded-lg shadow-lg z-10 p-6">
            <div className="flex items-center mb-4">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                (studentToDelete as any)._isInactive ? 'bg-red-100' : 'bg-orange-100'
              }`}>
                <Trash2 className={`w-5 h-5 ${
                  (studentToDelete as any)._isInactive ? 'text-red-600' : 'text-orange-600'
                }`} />
              </div>
            ...
            <div className="flex justify-end space-x-2">
              ...
              <button
                ...
                disabled={deletingStudent}
                className={`px-4 py-2 rounded-md text-white text-sm hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center ${
                  (studentToDelete as any)._isInactive ? 'bg-red-600 hover:bg-red-700' : 'bg-orange-600 hover:bg-orange-700'
                }`}
              >
                {deletingStudent ? (
                  <>
