        ...
        <div className="w-full max-w-5xl mx-auto md:p-8 p-6 space-y-8">
            ...
            <div>
                <h1 className="text-3xl font-black text-slate-800 flex items-center gap-3 tracking-tight">
                    <div className="w-12 h-12 bg-rose-50 rounded-2xl flex items-center justify-center shadow-sm border border-rose-100/50">
                        <BookOpen className="w-6 h-6 text-rose-600" />
                    </div>
                    My Assignments
                </h1>
                <p className="text-sm font-medium text-slate-500 mt-2 tracking-wide">Track homework, assignments and submissions</p>
            </div>
            ...
            {!loading && summary && (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    {[
                        { label: 'Total', value: summary.total, color: 'from-slate-50 to-slate-100/50 border-slate-100 text-slate-800 label-slate-500' },
                        { label: 'Submitted', value: summary.submitted, color: 'from-emerald-50 to-emerald-100/50 border-emerald-100 text-emerald-800 label-emerald-600' },
                        { label: 'Pending', value: summary.pending, color: 'from-amber-50 to-amber-100/50 border-amber-100 text-amber-800 label-amber-600' },
                        { label: 'Missing', value: summary.missing, color: 'from-red-50 to-red-100/50 border-red-100 text-red-800 label-red-600' },
                    ].map(s => {
                        ...
                        return (
                            <div key={s.label} className={`bg-gradient-to-br ${bgFrom} ${bgTo} border ${border} p-5 rounded-[20px] shadow-sm flex flex-col items-center justify-center text-center hover:shadow-md transition-shadow`}>
                                ...
                                <p className={`text-[10px] font-bold uppercase tracking-widest mt-1.5 ${label}`}>{s.label}</p>
                            </div>
                        )
                    })}
                </div>
            ...
            <div className="flex gap-2.5 flex-wrap">
                {[
                    ...
                    <button
                        ...
                        className={`px-5 py-2 rounded-full text-[11px] font-bold uppercase tracking-widest transition-all duration-300 ${filter === f.key
