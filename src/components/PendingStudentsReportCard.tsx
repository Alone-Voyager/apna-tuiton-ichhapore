  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || 'Failed to generate pending students report.');
      }
      ...
      let filename = `pending_students_${fromDate}_to_${toDate}.csv`;
      ...
      console.error('Error downloading pending students CSV:', err);
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 font-medium mt-0.5">
              Export unpaid &amp; outstanding student fee records for the selected period (.CSV format)
            </p>
          </div>
        </div>
      </div>

      {/* Date Range Inputs & Action Button */}
      <div className="bg-slate-50/80 p-4 sm:p-5 rounded-2xl border border-slate-200/80 space-y-4">
        ...
        {!isDatesPresent && (
          <p className="text-[11px] font-semibold text-slate-400">
            💡 Select both <span className="text-slate-700">From Date</span> and <span className="text-slate-700">To Date</span> to enable the report download.
          </p>
        )}
